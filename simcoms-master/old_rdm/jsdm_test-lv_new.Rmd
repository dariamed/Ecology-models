...
fit_gjam<-function(data, it=2500,burn=500 , name="./gjam_models/temp.rda",interact=diag(ncol(data$Y))){
  ...
  postL<-apply(mod_gjam1$chains$sgibbs[burn:dim(mod_gjam1$chains$sgibbs)[1],], 2, quantile,0.05)
...
data<-sim_data$EnvEvenSp5
#Rho_sign<-fit_gjam(data,10000,5000,"./gjam_models/gjam5env.rda")
Rho_sign<-load_gjam(data,10000,5000,name="./gjam_models/gjam5env.rda")
...
#setwd("~/Tesi/Code/Ecology-models-master/simcoms-master/ExampleFiles")
fit_hmsc<-function(data,label="Fit",nsamples = 3000,nchains=2,name="./HMmodels/hmtemp.rda" ){
  if (label=="Fit"){
...
hm_inter<-function(mod, nchains=2,nsamples = 1000, interact=diag(ns)){
  ...
  Toplot_T<-postRMean*(!(postTUp>0 & postTLo<0))
  ...
  title("Partial correlation non zero")
  corrplot(interact, diag = FALSE, order = "original",tl.pos = "ld", tl.cex = 0.5, method = "color",col=cols(200), type = "lower")
  title("True interactions")
...
Rho_sign<-hm_inter(hm_mod, nsamples = 3000,nchains=2)
...
Rho_sign(data,5000,500,name="./gjam_models/gjam10env.rda")
...
j_metric_comp_fDense5<-metrics_jsdm(cmp_fd5,comp = comp_inter[[16]],fac = fac_inter[[16]],only_env = F)
cat("\n")
cat(sprintf("Success rate for non-interacting: %s\n", round(j_metric_comp_fDense5$success_env,3)))
#cat(sprintf("Success rate for competition: %s\n", round(j_metric_comp_fDense5$success_comp,3)))
cat(sprintf("Success rate for facilitation: %s\n", round(j_metric_comp_fDense5$success_fac,3)))
...
#load("model-2019-04-17-14-30-01.rda")
# cmp_fd5 <- load_object("model-2019-04-18-01-55-45.rda")
# 
# summary(cmp_fd5)
# jsdm_conv(cmp_fd5)
# cmp_fd5$mcmc.info[1:7]
# 
# j_metric_comp_fDense5<-metrics_jsdm(cmp_fd5,comp = comp_inter[[16]],only_env = F)
# # cat("Success rate ")
# # j_metric_compSparse10$success_env
# # j_metric_compSparse10$success_comp
# # 
...
# 
# ######################################################Prepare data
# data<-sim_data$sim_data$FacCompDenseSp5
# data <- list(
#   Y = subset(data, select = -env),
#   X = cbind(1, scale(poly(data$env, 2))),
#   covx = cov(cbind(1, scale(poly(data$env, 2)))),
...
# )
# 
# plot_cor_jsdm(cmp_fd5,data$Y,(-1)*comp_inter[[14]])
...
h_metric_FacCompSparseSp5<-metrics_hmsc(Rho_sign,comp=comp_inter[[19]],fac= fac_inter[[19]] ,only_env = F)
cat("\n")
