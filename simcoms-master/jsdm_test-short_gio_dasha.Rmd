...
fit_gjam<-function(data, it=2500,burn=500 , name="./gjam_models/temp.rda",interact=diag(ncol(data$Y))){
  ...
  p1<-gjamPredict(mod_gjam1,newdata)
  p1
  ...
  return(list(Rho_sign=R_sign,Tau_sign=Tau_sign,predict=p1))
...
pred_gj_mean <- mod_gjam$predict$sdList$yMu
pred_gj_05 <- mod_gjam$predict$piList$yLo
pred_gj_95 <- mod_gjam$predict$piList$yHi
...
# Gradient = constructGradient(m,focalVariable = "env")
# Gradient$XDataNew
# predY = predict(m, Gradient=Gradient)
# plotGradient(m, Gradient, pred=predY,measure = "Y")
#                  
# predY = predict(m, XData=Gradient$env, studyDesign=Gradient$studyDesignNew, ranLevels=Gradient$rLNew, expected=TRUE)
...
data<-sim_data$EnvEvenSp5
#hm_mod<-fit_hmsc(data,"Fit",name="./HMmodels/hm5env.rda" )
...
```{r response to environnment env5, include=TRUE}
# to be finished
#fundamental niche
np<-500
nspecies<-5
...
```{r beta coefs env5, include=TRUE}
...
beta_mean<-prepare_table(me5$mean$B,name="mean",class="jsdm")
beta_q25<-prepare_table(datab=me5$q2.5$B,name="low",class="jsdm")
beta_q97<-prepare_table(me5$q97.5$B,name="high",class="jsdm")
...
beta_mean_raw<- apply(gj_mod$chains$bgibbsUn,2,mean)
beta_q25_raw<- apply(gj_mod$chains$bgibbsUn,2,quantile,0.025)
beta_q97_raw<- apply(gj_mod$chains$bgibbsUn,2,quantile,0.975)
...
beta_hm<-array(NA,dim=c(hm_mod$nc,hm_mod$ns,2*hm_mod$samples)) #2 is nchains
for(k in 1:(2*hm_mod$samples)){
    if(k<=hm_mod$samples){beta_hm[,,k] <- hm_mod$postList[[1]][[k]]$Beta
    }else {beta_hm[,,k] <- hm_mod$postList[[2]][[k-hm_mod$samples]]$Beta}
...
beta_mean<- prepare_beta_hmsc(datab=beta_mean_raw,name="mean", class="hmsc",vcol=hm_mod$spNames,vrow=hm_mod$covNames)
beta_q25<- prepare_beta_hmsc(beta_q25_raw,"low", "hmsc",vcol=hm_mod$spNames,vrow=hm_mod$covNames)
beta_q97<- prepare_beta_hmsc(beta_q97_raw,"high", "hmsc",vcol=hm_mod$spNames,vrow=hm_mod$covNames)
...
q<- p + geom_point( size = 2) +xlab("Environmental gradient")+ylab("Coefficients")+scale_y_discrete(
  ...
  geom_errorbarh(aes(xmax = high, xmin = low, height = .3))+ facet_wrap(~name,scales = "free")+
...
beta_mean_raw<- apply(beta_hm,c(1,2),mean)
beta_mean_q25<- apply(beta_hm,c(1,2),quantile,0.025)
beta_mean_q97<- apply(beta_hm,c(1,2),quantile,0.975)


beta_mean<- prepare_beta_gjam(beta_mean_raw,"mean", "hmsc")
beta_q25<- prepare_beta_gjam(beta_q25_raw,"low", "hmsc")
beta_q97<- prepare_beta_gjam(beta_q97_raw,"high", "hmsc")
