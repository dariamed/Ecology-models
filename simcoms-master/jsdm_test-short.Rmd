...
plot( d$X[,2],pnorm(prediction_means$EnvEvenSp5[,1]))
lines(dnorm(d$X[,2],2,20))

require(reshape2)
ggplot(data = melt(me5$sims.list$B[,,1])) + geom_boxplot


boxplot(me5$sims.list$B[,1,1])
abline(h=quantile(X,0.25),col="red",lty=2)
abline(h=quantile(X,0.75),col="red",lty=2)


plot( d$X[,2],pnorm(prediction_sds$EnvEvenSp5[,1]))


x <-d$X[,2]
y <- dnorm(x, mean=2, sd=20)
plot(d$X[,2], y, type="l", lwd=2)

plot(d$X[,2], type="l", lwd=2)

plotQuant(me5$sims.list$B)

library(ggplot2)
ggplot(stack(as.data.frame(me5$sims.list$B[,,1])), aes(x = ind, y = values)) +
  geom_boxplot()


gj_mod$chains$bgibbsUn
