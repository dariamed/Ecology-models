...
load_gjam<-function(data,it=2500,burn=500,name="./gjam_models/temp.rda",interact=diag(ncol(data$Y))){
  ...
  acfplot(mcmc(gj_mod$chains$sgibbs))
  acfplot(mcmc(gj_mod$chains$bgibbs))
...
hm_conv<-function(mod){
  ...
  acfplot(codaList$Beta)
  acfplot(codaList$Omega[[1]])
...
```{r hmsc study of interactions, include=TRUE,results=F}
...
hm_inter<-function(mod, nchains=2,nsamples = 1000, interact=diag(ns)){
  ...
  title("R non zero value")
  corrplot(Tau_n, diag = FALSE, order = "original",tl.pos = "ld", tl.cex = 0.5, method = "color",col=cols(200), type = "lower")
  title("Partial correlation non zero value")
  corrplot(interact, diag = FALSE, order = "original",tl.pos = "ld", tl.cex = 0.5, method = "color",col=cols(200), type = "lower")
  title("True interactions")
  
  return(list(Rho_sign=Toplot_R,Tau_sign=Tau_n))
}
...
```{r gjam en20 dim red, include=TRUE}
# 
# data<-sim_data$EnvEvenSp20
# 
...
```{r model jsdm fd10, echo=FALSE,include=TRUE}
...
```{r model2 hmsc, echo=FALSE,include=TRUE}
data<-sim_data$FacDenseSp10
...
```{r model jsdm fd20, echo=FALSE,include=TRUE}
#load("model-2019-04-10-15-13-15.rda")
...
```{r model jsdm fs10, echo=FALSE,include=TRUE}
#load("model-2019-04-10-15-13-15.rda")
...
```{r model jsdm fs20, echo=FALSE,include=TRUE}
#load("model-2019-04-10-15-13-15.rda")
...
```{r model jsdm cmpd5, echo=FALSE,include=TRUE}
#load("model-2019-04-10-15-13-15.rda")
...
```{r model jsdm cmpd10, echo=FALSE,include=TRUE}
#load("model-2019-04-10-15-13-15.rda")
...
```{r model jsdm cmpd20, echo=FALSE,include=TRUE}
#load("model-2019-04-10-15-13-15.rda")
...
```{r model jsdm cmps5, echo=FALSE,include=TRUE}
...
```{r model jsdm cmps10, echo=FALSE,include=TRUE}
#load("model-2019-04-17-14-30-01.rda")
...
```{r hmsc cmps10, results=F, include=TRUE}
data<-sim_data$CompSparseSp10
...
```{r model jsdm cmps20, echo=FALSE,include=TRUE}
#load("model-2019-04-17-14-30-01.rda")
...
```{r model jsdm cmp_facd5, echo=FALSE,include=TRUE}
...
```{r model jsdm cmp_facd10, echo=FALSE,include=TRUE}
...
```{r gjam cmp_facd10,include=TRUE}
data<-sim_data$FacCompDenseSp10
...
```{r hmsc cmp_facd10,include=TRUE}
...
```{r model jsdm cmp_facd20, echo=FALSE,include=TRUE}
...
```{r gjam cmp_facd20,include=TRUE}
data<-sim_data$FacCompDenseSp20
...
```{r hmsc cmp_facd20,include=TRUE}
data<-sim_data$FacCompDenseSp20
...
```{r model jsdm cmp_facs5, echo=FALSE,include=TRUE}
...
```{r model jsdm cmp_facs10, echo=FALSE,include=TRUE}
...
```{r gjam cmp_facs10,include=TRUE}
data<-sim_data$FacCompSparseSp10
...
```{r hmsc cmp_facs10,include=TRUE}
data<-sim_data$FacCompSparseSp10
...
```{r model jsdm cmp_facs20, echo=FALSE,include=TRUE}
...
```{r hmsc cmp_facs20,results=FALSE, include=TRUE}
data<-sim_data$FacCompSparseSp20
