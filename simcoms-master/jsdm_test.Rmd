...
Here are some warnings " In get(x, envir = ns) : internal error -5 in R_decompress1"? 

```{r hmsc, include=TRUE, results=FALSE}
#on mac install gcc, gfortran from source(install separately).
...
m = Hmsc(Y=Y, X=X, XFormula = ~env+ env2)
m = sampleMcmc(m, 1000, thin=10, adaptNf=c(2000,2000), nChains=2 )
