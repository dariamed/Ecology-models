...
########GJAM###################################################
  gjmod_1<-gjmod$m1
  beta_mean_raw<- apply(gjmod_1$chains$bgibbsUn,2,mean)
  beta_q25_raw<- apply(gjmod_1$chains$bgibbsUn,2,quantile,0.025)
  beta_q97_raw<- apply(gjmod_1$chains$bgibbsUn,2,quantile,0.975)
...
gjam_dim_red_5<-function(datab,ng=2500, burnin=500){
  ...
  J<- ncol(datab) - 1
  mu<-array(NA,dim=c(n,J,ng))
  for(k in 1:ng){
      for(j in 1:J){
    
         mu[,j,k] <- pnorm(x%*%mod_gjam_red_1$chains$bgibbs[k,(3*(j-1)+1):(3*j)])
  ...
  return(list(Rho_sign_d=Sigma_sign,Tau_sign_d=INVSigma_sign,k=kgibbs, predict=mu))
}
...
gjam_dim_red<-function(datab,ng=2500, burnin=500,r=1,regime="Load",name="./gjam_models/tmp.rda"){
  ydata<-subset(datab, select = -env)
  xdata<-scale(poly(datab$env, 2))
  ns<- ncol(ydata)
  N<-ncol(ydata)-1
  ...
  return(list(Rho_sign_d=Sigma_sign,Tau_sign_d=INVSigma_sign,r_opt=r, k=kgibbs , predict=mu))
}
#r-opt=7
S<- gjam_dim_red(sim_data$EnvEvenSp10,ng=2500, burnin=500,r=7)
...
#r-opt=15
S<- gjam_dim_red(sim_data$EnvEvenSp20,ng=2500, burnin=500,r=15)
...
S<- gjam_dim_red_5(sim_data$FacDenseSp5,ng=5500, burnin=1000)
...
#r-opt=7
S<- gjam_dim_red(sim_data$FacDenseSp10,ng=2500, burnin=500,r=7)
pg<- comp.psm(S$k)
...
#r-opt=15
S<- gjam_dim_red(sim_data$FacDenseSp20,ng=2500, burnin=500,r=15)
##Prediction
pred_gj_mean <-apply(S$predict, 1:2,mean)
pred_gj_05 <- apply(S$predict, 1:2,quantile,0.05)
pred_gj_95 <- apply(S$predict, 1:2,quantile,0.95)
...
S<- gjam_dim_red_5(sim_data$FacSparseSp5,ng=2500, burnin=1000)
...
#r-opt=5
S<- gjam_dim_red(sim_data$FacSparseSp10,ng=2500, burnin=500,r=5)
...
#r-opt=15
S<- gjam_dim_red(sim_data$FacSparseSp20,ng=2500, burnin=500,r=15)
...
S<- gjam_dim_red_5(sim_data$CompDenseSp5,ng=2500, burnin=1000)
