...
j_metric_facSparse10<-metrics_jsdm(mfs10,fac = fac_inter[[8]],only_env = F)
cat("\n")
...
for(i in 1:length(AUCs)){
  ...
  if(length(grep("_gdr_",names(AUCs)[[i]]))>0)  table[(k+1):(k+length(AUCs[[i]])),"Model"]<-rep("DR_GJAM",length(AUCs[[i]]))
...
table[nrow(table)+1,]<-c(-1,"DR_GJAM", "Env","Dense")
...
p<-ggplot(data=table)+geom_boxplot(aes(y=as.numeric(AUC),x=Filtering,fill=Model),position = position_dodge(1,preserve = "single"))+
  ylim(c(0,1)) +facet_grid(Density~.,scales = "free")+ theme(axis.text.x = element_text(angle = 45, hjust = 1,size = 10),strip.text = element_text(size = 15),legend.position = "top")+ylab("AUC")+ scale_x_discrete(name="Filtering", breaks=c("Env","Fac","Comp","C+F"),
                   labels=c("Environmental","Facilitation","Competition","Comp.+Fac."))+
ggtitle("Accuracy of model predictions averaged across communities") +theme_bw()
p
